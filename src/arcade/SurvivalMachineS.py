...
class gradientState(object):
	...
	if self.done != True:
	    ...
	    net.loadWeightsFromDisk(_path + b)
	    self.done = True
...
class testState(object):
	...
	if len(net.nextChoice) == 0:
        ...
        net.discount = 75
	for i in range(len(net.hiddenLayer)):
	    net.hiddenLayer[i].maxSigs = net.discount
	...
	sigs = net.sigmoids
	results = net.results
	aneal = float(T) / float(steps)
	
	for i in range(net.discount):
	    if i >= len(sigs):
	       return
	    rand = np.random.rand() 
	    ...
	    if rand < aneal:
		continue
	    test = sigs[self.id-1][i]

	    omitList = []
	    for k in range(len(sigs)):
	        if sigs[k][i] > test and k != (self.id-1):
		    omitList.append(i)
	    if len(omitList) == (len(sigs) -1):
		continue
	    result = results[i]
	    frame = frames[i]
	    LearningRate = np.random.rand() * .05 + .05
	    for k in range(len(perceptrons)):
		if np.isin(k, omitList):
		    continue
		sig = sigs[k][i]
	    	if perceptrons[k] == c1:
		    corrected = perceptrons[k].rewardDeltaRC(result, sig, LearningRate)
	    	else:
		    corrected = perceptrons[k].punishDeltaRC(result, sig, LearningRate)
	    for j in range(len(hiddenLayer)):
		if np.isin(hiddenLayer[j].getClass(), omitList):
		    continue
	    	if hiddenLayer[j].getClass() == c1:
		    hiddenLayer[j].rewardDeltaRI(frame, LearningRate,i)
	    	else:
		    hiddenLayer[j].punishDeltaRI(frame, LearningRate,i)
